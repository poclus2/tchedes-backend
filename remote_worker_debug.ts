    ...
    try {
        ...
        await db.kycSession.update({
            ...
            data: {
                ...
                extracted_fields: decision.merged_fields as any,
        ...
        await db.auditLog.create({
            data: {
                ...
                reason: decision.reasons.length > 0 ? decision.reasons.join(', ') : 'Automated approval',
                engine_version: `${DecisionEngine.ENGINE_VERSION} | OCR: ${frontOcr.engine_meta.ocr_provider}@${frontOcr.engine_meta.ocr_version}`
            }
        });

        // 6. [Phase 1.5] Enqueue Webhook Dispatch
        try {
